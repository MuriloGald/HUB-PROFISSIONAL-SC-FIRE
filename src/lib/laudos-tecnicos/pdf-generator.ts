 ...
 * (IN 04) — Alarme, Iluminação de Emergência e Estanqueidade da Rede de Gás.
 * Estrutura narrativa portada de laudos reais emitidos por terceiros
 * ("Laudos das inspeções/"), com cabeçalho e identidade visual SC Fire
 * (drawCabecalhoInstitucional, mesmo padrão dos demais módulos do hub).
 */

import { jsPDF } from "jspdf";
import { applyPlugin } from "jspdf-autotable";
import { drawCabecalhoInstitucional, carregarImagemComoDataUrl, COR_CINZA_INSTITUCIONAL, MARGIN_TOP, MARGIN_LEFT, MARGIN_RIGHT, PAGE_WIDTH, desenharTabelaRotulos } from "../shared/pdf-branding";
...
import { resultadoAlarme, resultadoIluminacao, resultadoGas } from "./resultado";
...
/** Gera o PDF do Laudo Técnico (Alarme/Iluminação/Gás) e dispara o download. */
export async function gerarPdfLaudoTecnico(state: LaudoTecnicoWizardState): Promise<string> {
  ...
  const rt = RESPONSAVEIS_TECNICOS[state.rt_selecionado || "rt1"];
  ...
  y = desenharTituloSecao(doc, y, "1. Apresentação");
  y = desenharParagrafo(doc, y, config.apresentacao);

  y = desenharTituloSecao(doc, y, "2. Objetivo");
  y = desenharParagrafo(doc, y, config.objetivo(descricaoImovel(state)));

  y = desenharTituloSecao(doc, y, "3. Referências Normativas");
  y = desenharParagrafo(doc, y, config.referencias);

  y = desenharTituloSecao(doc, y, "4. Descritivo do Instrumento Utilizado na Medição");
  ...
  if (tipo === "gas") {
    y = desenharTituloSecao(doc, y, "5. Condições do Teste");
    y = desenharTabelaRotulos(doc, y, [
      [
  ...
  y = desenharTituloSecao(doc, y, `${tipo === "gas" ? "6" : "5"}. Resultados das Medições`);
  ...
  y = desenharTituloSecao(doc, y, `${tipo === "gas" ? "7" : "6"}. Conclusão`);
  ...
  doc.setFontSize(8);
  doc.setTextColor(...COR_CINZA_INSTITUCIONAL);
  doc.text(`Responsável Técnico — ${rt.classe}`, margin, y + 9);
