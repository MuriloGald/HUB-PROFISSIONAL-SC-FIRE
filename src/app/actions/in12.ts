...
/** Salva (cria ou atualiza) o relatório de comissionamento de alarme de incêndio (IN 12). */
export async function salvarAlarmeIn12(state: AlarmeIn12State) {
  ...
  if (!state.laudoId) {
    ...
    const { count } = await supabase
      ...
      .eq("tipo_documento", "IN12")
      .gte("created_at", `${ano}-01-01`)
      .lt("created_at", `${ano + 1}-01-01`);

    codigo = `ALARM-${ano}-${((count ?? 0) + 1).toString().padStart(3, "0")}`;
  }

  const dados: AlarmeIn12State = {
    ...state,
    codigo,
...
export async function listarAlarmeIn12() {
  const supabase = await createClient();
...
export async function buscarAlarmeIn12(id: string) {
  const supabase = await createClient();
...
export async function excluirAlarmeIn12(id: string) {
  const supabase = await createClient();
