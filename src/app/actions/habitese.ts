...
export interface ClienteImovelInput {
  id?: string;
  cnpj?: string;
  cpf?: string;
  razao_social: string;
...
/** Cria ou atualiza um proprietario/responsavel pelo imovel (tipo = 'imovel'). */
export async function salvarClienteImovel(input: ClienteImovelInput) {
...
export async function listarClientesImovel() {
  const supabase = await createClient();
...
/**
 * Exclui um proprietario permanentemente. ATENÇÃO: laudos.cliente_id tem
 * "on delete cascade" — isso apaga junto todos os termos vinculados a este
 * proprietario. A UI deve confirmar isso explicitamente antes de chamar.
 */
export async function excluirClienteImovel(id: string) {
  const supabase = await createClient();
