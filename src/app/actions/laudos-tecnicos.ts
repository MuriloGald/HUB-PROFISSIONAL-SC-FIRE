...
/** Salva (cria ou atualiza) um Laudo Técnico (Alarme/Iluminação/Gás), gerando um código LT-AAAA-NNN quando novo. */
export async function salvarLaudoTecnico(state: LaudoTecnicoWizardState) {
  ...
  if (!state.laudoId) {
    ...
    const { count } = await supabase
      ...
      .eq("tipo_documento", "LaudoTecnico")
      .gte("created_at", `${ano}-01-01`)
      .lt("created_at", `${ano + 1}-01-01`);

    codigo = `LT-${ano}-${((count ?? 0) + 1).toString().padStart(3, "0")}`;
  }

  const dados: LaudoTecnicoWizardState = { ...state, codigo, data_emissao: state.data_emissao || new Date().toISOString() };
  delete dados.laudoId;

  const payload = {
    cliente_id: state.cliente_id,
    tipo_documento: "LaudoTecnico" as const,
