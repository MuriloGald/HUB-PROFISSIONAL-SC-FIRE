...
import { ShowerHead, PlusCircle, ListChecks } from "lucide-react";
import { listarChuveiros } from "@/app/actions/in15";

export default async function In15DashboardPage() {
  const { data: laudos } = await listarChuveiros();

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-white font-display">IN 15 — Chuveiros Automáticos</h1>
        <p className="text-sm text-gray-400 mt-1">Anexo B (Comissionamento) e Anexo C (Inspeção), NBR 10.897, cabeçalho oficial do CBMSC.</p>
      </div>
      ...
      <section className="grid grid-cols-1 gap-4">
        <div className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.06] flex items-center justify-between">
          ...
          <ShowerHead className="w-8 h-8 text-blue-500/30" />
        </div>
      </section>

      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Link
          href="/documentos/in15/novo"
