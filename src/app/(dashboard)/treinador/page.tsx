...
export default async function TreinadorDashboardPage() {
  const { data: cursos } = await listarCursosTreinador();

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-white font-display">Treinador — Roteiro de Aula</h1>
        <p className="text-sm text-gray-400 mt-1">
          Escolha o curso pra ver o roteiro completo de cada aula (contexto, explicação, recurso, verificação e fechamento).
        </p>
      </div>

      {cursos.length === 0 && (
        <div className="p-6 rounded-xl bg-white/[0.02] border border-white/[0.06] text-sm text-gray-400">
          Nenhum curso cadastrado ainda em Treinamentos.
        </div>
      )}
