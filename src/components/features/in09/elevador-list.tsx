...
import { excluirComissionamentoElevador } from "@/app/actions/in09";
...
export function ElevadorList({ laudos }: { laudos: Laudo[] }) {
  ...
  const filtrados = useMemo(() => {
    ...
    return laudos.filter((l) => {
      const dados = l.dados as ComissionamentoElevadorState;
  ...
  async function handleExcluir(id: string) {
    ...
    await excluirComissionamentoElevador(id);
    router.refresh();
  }

  async function handleBaixar(dados: ComissionamentoElevadorState) {
    setErro(null);
    try {
      const { gerarPdfComissionamentoElevador } = await import("@/lib/in09/pdf-generator");
      await gerarPdfComissionamentoElevador(dados);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white font-display">Consultar Relatórios do Elevador de Emergência</h1>
          <p className="text-sm text-gray-400 mt-1">Histórico de Relatórios de Comissionamento do Elevador (IN 09, Anexo E) já emitidos.</p>
        </div>
        <button
          onClick={() => router.push("/documentos/in09/novo")}
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar por código (ELEV-2026-001), responsável ou RE..."
            ...
            <div key={l.id} className="rounded-xl bg-white/[0.02] border border-white/[0.08] border-l-4 border-l-red-500 p-4 flex items-center justify-between flex-wrap gap-4">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  <strong className="text-gray-300">RE:</strong> {dados.re || "-"} | <strong className="text-gray-300">Qtd. elevadores:</strong> {dados.qtd_elevadores || "-"}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => router.push(`/documentos/in09/novo?editarId=${l.id}`)}
