...
import { excluirControleFumaca } from "@/app/actions/in10";
...
export function FumacaList({ laudos }: { laudos: Laudo[] }) {
  ...
  const filtrados = useMemo(() => {
    ...
    return laudos.filter((l) => {
      const dados = l.dados as ControleFumacaState;
      return dados.proprietario_nome?.toLowerCase().includes(t) || dados.codigo?.toLowerCase().includes(t) || dados.re?.toLowerCase().includes(t);
    });
  }, [laudos, termo]);

  async function handleExcluir(id: string) {
    if (!confirm("Tem certeza que deseja excluir este relatório? Essa ação não pode ser desfeita.")) return;
    await excluirControleFumaca(id);
    router.refresh();
  }

  async function handleBaixar(dados: ControleFumacaState) {
    setErro(null);
    try {
      const { gerarPdfComissionamentoFumaca, gerarPdfInspecaoFumaca } = await import("@/lib/in10/pdf-generator");
      if (dados.fluxo === "inspecao") {
        await gerarPdfInspecaoFumaca(dados);
      } else {
        await gerarPdfComissionamentoFumaca(dados);
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white font-display">Consultar Relatórios do Sistema de Controle de Fumaça</h1>
          <p className="text-sm text-gray-400 mt-1">Histórico de Relatórios de Comissionamento (Anexo B) e Inspeção (Anexo C) da IN 10 já emitidos.</p>
        </div>
        <button
          onClick={() => router.push("/documentos/in10/novo")}
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar por código (FUM-2026-001), proprietário ou RE..."
          ...
          return (
            <div key={l.id} className="rounded-xl bg-white/[0.02] border border-white/[0.08] border-l-4 border-l-red-500 p-4 flex items-center justify-between flex-wrap gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="px-2 py-0.5 text-[10px] font-bold text-blue-400 bg-blue-500/10 border border-blue-500/20 rounded-full uppercase">
                    {dados.fluxo === "inspecao" ? "Anexo C" : "Anexo B"}
                  </span>
                  <h3 className="text-white font-semibold">{dados.proprietario_nome || "Sem proprietário"}</h3>
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => router.push(`/documentos/in10/novo?editarId=${l.id}`)}
