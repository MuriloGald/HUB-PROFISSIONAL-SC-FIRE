...
export function LaudosList({ laudos }: { laudos: Laudo[] }) {
  ...
  const filtrados = useMemo(() => {
    ...
    return laudos.filter((l) => {
      ...
      return dados.cliente?.razao_social?.toLowerCase().includes(t) || dados.codigo?.toLowerCase().includes(t) || dados.tituloDocumento?.toLowerCase().includes(t);
    });
  }, [laudos, termo]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white font-display">Laudos Técnicos — SAVE 23</h1>
          <p className="text-sm text-gray-400 mt-1">Orientações técnicas já emitidas (Art. 6º, IN 23/CBMSC).</p>
        </div>
        <button
          onClick={() => router.push("/relatorios/save-in23/laudos/novo")}
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar por código (L-2026-001), edificação ou título..."
      ...
      <div className="grid gap-3">
        {filtrados.length === 0 && (
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] text-center py-12">
            ...
            <p className="text-xs text-gray-500 mt-1">{termo ? "Tente buscar por outro termo." : "Gere o primeiro laudo no botão acima."}</p>
          </div>
        )}
            ...
            <div key={l.id} className="rounded-xl bg-white/[0.02] border border-white/[0.08] border-l-4 border-l-red-500 p-4 flex items-center justify-between flex-wrap gap-4">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  <strong className="text-gray-300">Título:</strong> {dados.tituloDocumento || "N/A"} |{" "}
                  <strong className="text-gray-300">Revisão:</strong> {dados.revisao || "N/A"}
                </p>
              </div>
