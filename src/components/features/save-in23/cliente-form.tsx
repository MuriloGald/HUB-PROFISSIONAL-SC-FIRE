...
import { salvarClienteSave23 } from "@/app/actions/save-in23";
...
interface ClienteFormProps {
  cliente?: Cliente;
  /** Para onde ir apos salvar (ex: para retomar o wizard de vistoria/laudo). Default: lista de clientes. */
  redirectTo?: string;
}
...
export function ClienteForm({ cliente, redirectTo }: ClienteFormProps) {
  ...
  function calcularAvisos(): Record<string, string> {
    ...
    const camposObrigatorios: { key: string; label: string; valor: string }[] = [
      { key: "razaoSocial", label: "Nome da edificação", valor: razaoSocial },
      { key: "nomeResponsavel", label: "Responsável pelo imóvel", valor: nomeResponsavel },
      ...
      { key: "cidade", label: "Cidade / Município", valor: cidade },
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSalvando(true);
    const result = await salvarClienteSave23({
      id: cliente?.id,
      cnpj: cnpj || undefined,
      cpf: cpf || undefined,
    ...
    router.push(
      redirectTo ? `${redirectTo}${redirectTo.includes("?") ? "&" : "?"}clienteId=${result.data.id}` : "/relatorios/save-in23/clientes"
    );
    router.refresh();
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl bg-white/[0.02] border border-white/[0.08] p-6 space-y-6">
      ...
      <div className="space-y-1.5">
        <label className={labelClass}>Nome da Edificação</label>
      ...
      <div className="space-y-1.5">
        <label className={labelClass}>Responsável pelo Imóvel</label>
      ...
      <h3 className="text-sm font-bold text-white border-l-2 border-red-500 pl-2 pt-2">Dados da Edificação (IN 23)</h3>
      ...
      <div className="flex justify-end gap-3 pt-2">
        <button
          type="button"
          onClick={() => router.push("/relatorios/save-in23/clientes")}
