...
import { excluirChuveiros } from "@/app/actions/in15";
...
export function ChuveirosList({ laudos }: { laudos: Laudo[] }) {
  ...
  const filtrados = useMemo(() => {
    ...
    return laudos.filter((l) => {
      const dados = l.dados as ChuveirosState;
      return dados.responsavel_imovel_nome?.toLowerCase().includes(t) || dados.codigo?.toLowerCase().includes(t) || dados.re?.toLowerCase().includes(t);
    });
  }, [laudos, termo]);

  async function handleExcluir(id: string) {
    if (!confirm("Tem certeza que deseja excluir este relatório? Essa ação não pode ser desfeita.")) return;
    await excluirChuveiros(id);
    router.refresh();
  }

  async function handleBaixar(dados: ChuveirosState) {
    setErro(null);
    try {
      const { gerarPdfComissionamentoChuveiros, gerarPdfInspecaoChuveiros } = await import("@/lib/in15/pdf-generator");
      if (dados.fluxo === "comissionamento") {
        await gerarPdfComissionamentoChuveiros(dados);
      } else {
        await gerarPdfInspecaoChuveiros(dados);
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white font-display">Consultar Relatórios de Chuveiros Automáticos</h1>
          <p className="text-sm text-gray-400 mt-1">Histórico de Relatórios de Comissionamento (Anexo B) e Inspeção (Anexo C) da IN 15 já emitidos.</p>
        </div>
        <button
          onClick={() => router.push("/documentos/in15/novo")}
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar por código (CHUV-2026-001), responsável ou RE..."
      ...
      <div className="grid gap-3">
        {filtrados.length === 0 && (
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] text-center py-12">
            ...
            <h3 className="text-gray-400 font-semibold">Nenhum relatório emitido ainda</h3>
            <p className="text-xs text-gray-500 mt-1">{termo ? "Tente buscar por outro termo." : "Gere o seu primeiro relatório no botão acima."}</p>
          </div>
        )}
            ...
            <div key={l.id} className="rounded-xl bg-white/[0.02] border border-white/[0.08] border-l-4 border-l-red-500 p-4 flex items-center justify-between flex-wrap gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="px-2 py-0.5 text-[10px] font-bold text-blue-400 bg-blue-500/10 border border-blue-500/20 rounded-full uppercase">
                    {dados.fluxo === "comissionamento" ? "Anexo B" : "Anexo C"}
                  </span>
                  <h3 className="text-white font-semibold">{dados.responsavel_imovel_nome || "Sem responsável"}</h3>
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => router.push(`/documentos/in15/novo?editarId=${l.id}`)}
