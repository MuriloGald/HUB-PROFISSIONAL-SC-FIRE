...
import type { ClienteEventoSnapshot } from "@/lib/laudos/types";

function clienteParaSnapshot(c: Cliente): ClienteEventoSnapshot {
...
interface StepClienteProps {
  ...
  onNext: (clienteId: string, cliente: ClienteEventoSnapshot) => void;
}

export function StepCliente({ clientes, clienteIdInicial, onNext }: StepClienteProps) {
  ...
  return (
    <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] p-6 space-y-6">
      <h3 className="text-lg font-bold text-white">Selecione o Cliente Responsável</h3>
      ...
      <div className="text-center">
        <Link
          href="/laudos/clientes/novo?redirectTo=/laudos/eventos/novo"
