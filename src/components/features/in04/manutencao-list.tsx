...
import { excluirVistoriaManutencao } from "@/app/actions/in04";
...
export function ManutencaoList({ laudos }: { laudos: Laudo[] }) {
  ...
  const filtrados = useMemo(() => {
    ...
    return laudos.filter((l) => {
      const dados = l.dados as unknown as VistoriaManutencaoState;
  ...
  async function handleExcluir(id: string) {
    ...
    await excluirVistoriaManutencao(id);
    router.refresh();
  }

  async function handleBaixar(dados: VistoriaManutencaoState) {
    setErro(null);
    try {
      const { gerarPdfVistoriaManutencao } = await import("@/lib/in04/pdf-generator");
      await gerarPdfVistoriaManutencao(dados);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white font-display">Consultar Vistorias de Manutenção do SMSCI</h1>
          <p className="text-sm text-gray-400 mt-1">Histórico de vistorias de manutenção (IN 04) já emitidas.</p>
        </div>
        <button
          onClick={() => router.push("/documentos/in04/novo")}
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar por código (VM-2026-001) ou cliente..."
      ...
      <div className="grid gap-3">
        {filtrados.length === 0 && (
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] text-center py-12">
            ...
            <h3 className="text-gray-400 font-semibold">Nenhuma vistoria emitida ainda</h3>
            <p className="text-xs text-gray-500 mt-1">{termo ? "Tente buscar por outro termo." : "Gere a sua primeira vistoria no botão acima."}</p>
          </div>
        )}

        {filtrados.map((l) => {
          const dados = l.dados as unknown as VistoriaManutencaoState;
          const totalItens = dados.pavimentos?.reduce((n, p) => n + Object.values(p.itens).reduce((m, arr) => m + arr.length, 0), 0) ?? 0;
          return (
            <div key={l.id} className="rounded-xl bg-white/[0.02] border border-white/[0.08] border-l-4 border-l-red-500 p-4 flex items-center justify-between flex-wrap gap-4">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  <strong className="text-gray-300">Pavimentos:</strong> {dados.pavimentos?.length ?? 0} | <strong className="text-gray-300">Itens lançados:</strong> {totalItens}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => router.push(`/documentos/in04/novo?editarId=${l.id}`)}
