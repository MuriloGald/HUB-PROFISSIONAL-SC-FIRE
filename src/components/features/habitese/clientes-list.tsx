...
import { Search, Plus, Pencil, Trash2, FileSignature, PackageOpen, MapPin } from "lucide-react";
import { excluirClienteImovel } from "@/app/actions/habitese";
...
export function ClientesList({ clientes }: { clientes: Cliente[] }) {
  ...
  async function handleExcluir(c: Cliente) {
    const ok = confirm(
      `Excluir "${c.razao_social}" permanentemente?\n\nIsso também apaga TODOS os termos vinculados a este proprietário. Essa ação não pode ser desfeita.`
    );
    if (!ok) return;
    setExcluindo(c.id);
    await excluirClienteImovel(c.id);
    setExcluindo(null);
    router.refresh();
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white font-display">Proprietários (Habite-se)</h1>
          <p className="text-sm text-gray-400 mt-1">Gerencie os responsáveis pelo imóvel ou inicie um novo termo de entrega.</p>
        </div>
        <Link
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar por nome, CNPJ/CPF ou responsável..."
      ...
      <div className="grid gap-3">
        {filtrados.length === 0 && (
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] text-center py-12">
            ...
            <h3 className="text-gray-400 font-semibold">Nenhum proprietário encontrado</h3>
            <p className="text-xs text-gray-500 mt-1">{termo ? "Tente buscar por outro termo." : "Comece cadastrando um novo proprietário."}</p>
          </div>
        )}
        ...
        {filtrados.map((c) => (
          ...
          >
            ...
            <div className="flex gap-2">
              <Link
                href={`/habitese/responsaveis/${c.id}/editar`}
              ...
              <button
                onClick={() => router.push(`/habitese/novo?clienteId=${c.id}`)}
              ...
              >
                <FileSignature className="w-3.5 h-3.5" /> Novo Termo
              </button>
            </div>
          </div>
        ))}
