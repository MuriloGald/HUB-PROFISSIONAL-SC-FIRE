...
import { excluirComissionamentoSHP } from "@/app/actions/in07";
...
export function ShpList({ laudos }: { laudos: Laudo[] }) {
  ...
  const filtrados = useMemo(() => {
    ...
    return laudos.filter((l) => {
      const dados = l.dados as ComissionamentoSHPState;
  ...
  async function handleExcluir(id: string) {
    ...
    await excluirComissionamentoSHP(id);
    router.refresh();
  }

  async function handleBaixar(dados: ComissionamentoSHPState) {
    setErro(null);
    try {
      const { gerarPdfComissionamentoSHP } = await import("@/lib/in07/pdf-generator");
      await gerarPdfComissionamentoSHP(dados);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white font-display">Consultar Relatórios de Comissionamento SHP</h1>
          <p className="text-sm text-gray-400 mt-1">Histórico de Relatórios de Comissionamento/Inspeção do SHP (IN 07, Anexo C) já emitidos.</p>
        </div>
        <button
          onClick={() => router.push("/documentos/in07/novo")}
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar por código (SHP-2026-001), proprietário ou RE..."
            ...
            <div key={l.id} className="rounded-xl bg-white/[0.02] border border-white/[0.08] border-l-4 border-l-red-500 p-4 flex items-center justify-between flex-wrap gap-4">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  <strong className="text-gray-300">RE:</strong> {dados.re || "-"} | <strong className="text-gray-300">Vazão medida:</strong> {dados.vazao_medida || "-"} l/min
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => router.push(`/documentos/in07/novo?editarId=${l.id}`)}
