...
import { excluirRessarcimento } from "@/app/actions/in02";
...
export function RessarcimentoList({ laudos }: { laudos: Laudo[] }) {
  ...
  const filtrados = useMemo(() => {
    ...
    return laudos.filter((l) => {
      const dados = l.dados as RessarcimentoWizardState;
      return dados.requerente_nome?.toLowerCase().includes(t) || dados.codigo?.toLowerCase().includes(t);
    });
  }, [laudos, termo]);

  async function handleExcluir(id: string) {
    if (!confirm("Tem certeza que deseja excluir este requerimento? Essa ação não pode ser desfeita.")) return;
    await excluirRessarcimento(id);
    router.refresh();
  }

  async function handleBaixar(dados: RessarcimentoWizardState) {
    setErro(null);
    try {
      const { gerarPdfRessarcimento } = await import("@/lib/in02/pdf-generator");
      await gerarPdfRessarcimento(dados);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white font-display">Consultar Requerimentos de Ressarcimento</h1>
          <p className="text-sm text-gray-400 mt-1">Histórico de requerimentos de ressarcimento de multa (IN 02, Anexo K/L) já emitidos.</p>
        </div>
        <button
          onClick={() => router.push("/documentos/in02/ressarcimento/novo")}
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar por código (RES-2026-001) ou nome do requerente..."
      ...
      <div className="grid gap-3">
        {filtrados.length === 0 && (
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] text-center py-12">
            ...
            <h3 className="text-gray-400 font-semibold">Nenhum requerimento emitido ainda</h3>
            <p className="text-xs text-gray-500 mt-1">{termo ? "Tente buscar por outro termo." : "Gere o seu primeiro requerimento no botão acima."}</p>
          </div>
        )}
            ...
            <div key={l.id} className="rounded-xl bg-white/[0.02] border border-white/[0.08] border-l-4 border-l-red-500 p-4 flex items-center justify-between flex-wrap gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="px-2 py-0.5 text-[10px] font-bold text-blue-400 bg-blue-500/10 border border-blue-500/20 rounded-full uppercase">{dados.tipo}</span>
                  <h3 className="text-white font-semibold">{dados.requerente_nome || "Sem requerente"}</h3>
                </div>
                <p className="text-xs text-gray-400">
                  <strong className="text-gray-300">Multa:</strong> {dados.multa_numero || "-"} |{" "}
                  <strong className="text-gray-300">Valor:</strong> R$ {dados.multa_valor || "-"}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => router.push(`/documentos/in02/ressarcimento/novo?editarId=${l.id}`)}
