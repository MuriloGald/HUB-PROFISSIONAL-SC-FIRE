...
import { excluirRecurso } from "@/app/actions/in02";
...
export function RecursoList({ laudos }: { laudos: Laudo[] }) {
  ...
  const filtrados = useMemo(() => {
    ...
    return laudos.filter((l) => {
      const dados = l.dados as RecursoWizardState;
      return dados.autuado_nome?.toLowerCase().includes(t) || dados.codigo?.toLowerCase().includes(t);
    });
  }, [laudos, termo]);

  async function handleExcluir(id: string) {
    if (!confirm("Tem certeza que deseja excluir este recurso? Essa ação não pode ser desfeita.")) return;
    await excluirRecurso(id);
    router.refresh();
  }

  async function handleBaixar(dados: RecursoWizardState) {
    setErro(null);
    try {
      const { gerarPdfRecurso } = await import("@/lib/in02/pdf-generator");
      await gerarPdfRecurso(dados);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white font-display">Consultar Recursos</h1>
          <p className="text-sm text-gray-400 mt-1">Histórico de Formulários de Recurso (IN 02, Anexo J) já emitidos.</p>
        </div>
        <button
          onClick={() => router.push("/documentos/in02/recurso/novo")}
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar por código (REC-2026-001) ou nome do autuado..."
      ...
      <div className="grid gap-3">
        {filtrados.length === 0 && (
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] text-center py-12">
            ...
            <h3 className="text-gray-400 font-semibold">Nenhum recurso emitido ainda</h3>
            <p className="text-xs text-gray-500 mt-1">{termo ? "Tente buscar por outro termo." : "Gere o seu primeiro recurso no botão acima."}</p>
          </div>
        )}
            ...
            <div key={l.id} className="rounded-xl bg-white/[0.02] border border-white/[0.08] border-l-4 border-l-red-500 p-4 flex items-center justify-between flex-wrap gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="text-white font-semibold">{dados.autuado_nome || "Sem autuado"}</h3>
                </div>
                <p className="text-xs text-gray-400">
                  <strong className="text-gray-300">Auto de Infração:</strong> {dados.auto_infracao_numero || "-"} |{" "}
                  <strong className="text-gray-300">RE:</strong> {dados.imovel_re || "-"}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => router.push(`/documentos/in02/recurso/novo?editarId=${l.id}`)}
                ...
                <button
                  ...
                  title="Baixar Formulário de Recurso (PDF)"
