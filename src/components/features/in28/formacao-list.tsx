...
import { excluirRelatorioFormacao } from "@/app/actions/in28";
...
export function FormacaoList({ laudos }: { laudos: Laudo[] }) {
  ...
  const filtrados = useMemo(() => {
    ...
    return laudos.filter((l) => {
      const dados = l.dados as RelatorioFormacaoState;
  ...
  async function handleExcluir(id: string) {
    ...
    await excluirRelatorioFormacao(id);
    router.refresh();
  }

  async function handleBaixar(dados: RelatorioFormacaoState) {
    setErro(null);
    try {
      const { gerarPdfRelatorioFormacao } = await import("@/lib/in28/pdf-generator");
      await gerarPdfRelatorioFormacao(dados);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white font-display">Consultar Relatórios — Empresas de Formação</h1>
          <p className="text-sm text-gray-400 mt-1">Histórico de relatórios anuais de empresas de formação de brigadistas (IN 28, Anexo E).</p>
        </div>
        <button
          onClick={() => router.push("/documentos/in28/formacao/novo")}
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar por código (FORM-2026-001) ou razão social..."
          ...
          return (
            <div key={l.id} className="rounded-xl bg-white/[0.02] border border-white/[0.08] border-l-4 border-l-red-500 p-4 flex items-center justify-between flex-wrap gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => router.push(`/documentos/in28/formacao/novo?editarId=${l.id}`)}
