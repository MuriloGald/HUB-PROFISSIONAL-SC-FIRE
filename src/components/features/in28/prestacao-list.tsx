...
import { excluirRelatorioPrestacao } from "@/app/actions/in28";
...
export function PrestacaoList({ laudos }: { laudos: Laudo[] }) {
  ...
  const filtrados = useMemo(() => {
    ...
    return laudos.filter((l) => {
      const dados = l.dados as RelatorioPrestacaoState;
      return dados.razao_social?.toLowerCase().includes(t) || dados.codigo?.toLowerCase().includes(t);
    });
  }, [laudos, termo]);

  async function handleExcluir(id: string) {
    if (!confirm("Tem certeza que deseja excluir este relatório? Essa ação não pode ser desfeita.")) return;
    await excluirRelatorioPrestacao(id);
    router.refresh();
  }

  async function handleBaixar(dados: RelatorioPrestacaoState) {
    setErro(null);
    try {
      const { gerarPdfRelatorioPrestacao } = await import("@/lib/in28/pdf-generator");
      await gerarPdfRelatorioPrestacao(dados);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white font-display">Consultar Relatórios — Empresas de Prestação de Serviço</h1>
          <p className="text-sm text-gray-400 mt-1">Histórico de relatórios anuais de empresas de prestação de serviço de brigadistas (IN 28, Anexo F).</p>
        </div>
        <button
          onClick={() => router.push("/documentos/in28/prestacao/novo")}
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar por código (PREST-2026-001) ou razão social..."
            ...
            <div key={l.id} className="rounded-xl bg-white/[0.02] border border-white/[0.08] border-l-4 border-l-red-500 p-4 flex items-center justify-between flex-wrap gap-4">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  <strong className="text-gray-300">Ano:</strong> {dados.ano_referencia || "-"}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => router.push(`/documentos/in28/prestacao/novo?editarId=${l.id}`)}
